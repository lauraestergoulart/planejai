...
Este projeto foi desenvolvido a partir do desafio da [Digital Innovation One (DIO)](https://github.com/digitalinnovationone/planejai), como parte da trilha de aprendizado de Front-End com React e IA Generativa.
...
2. Instale as dependências (o projeto usa `pnpm`):

```bash
pnpm install
```

3. Crie uma chave de API gratuita no [Google AI Studio](https://aistudio.google.com/) e crie o arquivo `.env.local` na raiz do projeto com o conteúdo:
...
1. Na tela inicial, preencha o formulário de simulação até o fim (renda, custos fixos, dívidas, nome da meta, valor da meta e prazo em meses).
2. Ao concluir, você será redirecionado para a página de resultado, onde a IA gera o diagnóstico financeiro (com um carregamento tipo *skeleton* enquanto isso acontece).
3. Repita o processo mais de uma vez, com dados diferentes, para gerar várias simulações.
4. Clique em **"Histórico"** no cabeçalho para ver todas as simulações salvas.
5. Clique em **"Ver detalhes"** em uma simulação já processada e confirme que o diagnóstico aparece instantaneamente (sem nova chamada à IA).
6. Clique no botão de excluir em um dos cards, confirme a exclusão e veja que a simulação some da lista.
